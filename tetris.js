...
const RetroTetris = () => {
      ...
      if (!movePiece(0, 1)) {
        ...
        if (newBoard[0].some(cell => cell)) {
          ...
          if (completedRows > 0) {
            ...
            // Prüfe, ob 4 Reihen erreicht wurden
  ...
  if (showHighScores) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 p-4 font-mono">
        <div className="bg-gray-200 p-8 rounded-lg shadow-lg relative">
          <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-gray-300 px-4 py-1 rounded text-sm font-bold">
            GAME TOY
          </div>
          
          <div className="bg-green-100 p-6 rounded-sm shadow-inner">
            <h2 className="text-xl text-green-900 mb-4">High Scores</h2>
            <div className="space-y-2">
              {highScores.map((score, index) => (
                <div key={index} className="flex justify-between text-green-900">
                  <span>{index === 3 ? (
                    <input
                      type="text"
                      value={playerName}
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 p-4 font-mono select-none touch-none">
      <div className="bg-gray-200 p-8 rounded-lg shadow-lg relative">
        <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-gray-300 px-4 py-1 rounded text-sm font-bold">
          GAME TOY
        </div>
        ...
        <div className="bg-green-100 p-6 rounded-sm shadow-inner">
          <div className="mb-4 text-2xl text-green-900">Lines: {completedLines}</div>
          <div className="border-4 border-green-900 bg-green-200 p-1">
